...
public class CassPerfHectorBase {
    ...
    protected static void initializeHectorLib() {
        ...
        // healthCareKeyspace = HFactory.createKeyspace("Tutorial", hcSystemCluster, consistencyLevelPolicy);
        // see also me.prettyprint.cassandra.model.ConfigurableConsistencyLevelPolicy[Test] for details

        cassPerfCluster = HFactory.getOrCreateCluster(properties.getProperty("cluster.name", "CassPerfCluster"),
                properties.getProperty("cluster.hosts", "127.0.0.1:9160"));
//        hcSystemCluster = HFactory.getOrCreateCluster("HealthCareCluster", "10.12.2.209:9160");
        ConfigurableConsistencyLevel configConsistencyLevel = new ConfigurableConsistencyLevel();
        configConsistencyLevel.setDefaultReadConsistencyLevel(HConsistencyLevel.ONE);
        configConsistencyLevel.setDefaultWriteConsistencyLevel(HConsistencyLevel.ANY);


//        healthCareKeyspace = HFactory.createKeyspace(properties.getProperty("healthcare.keyspace", "HealthCare"), hcSystemCluster, ccl);
        cassPerfKeyspace = HFactory.createKeyspace("CassExp", cassPerfCluster, configConsistencyLevel);
